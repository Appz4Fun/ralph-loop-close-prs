...
def _filter_to_still_open_prs(pr_numbers: List[int]) -> List[int]:
    ...
    """
    # Performance Optimization: Checking multiple PR states via 'gh' CLI sequentially causes N+1 delays.
    # We use ThreadPoolExecutor to perform these checks concurrently, reducing total wait time.
    if not pr_numbers:
        return []

    def _worker(pr: int) -> Tuple[int, Any, bool]:
        try:
            return (pr, None, _pr_is_still_open(pr))
    ...
    with concurrent.futures.ThreadPoolExecutor(
        ...
        for pr, exc, still_open in executor.map(_worker, pr_numbers):
            if exc:
