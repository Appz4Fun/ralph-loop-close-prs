...
def _filter_to_still_open_prs(pr_numbers: List[int]) -> List[int]:
    ...
    def _check(pr: int) -> Tuple[int, Optional[bool], Optional[Exception]]:
        try:
    ...
    # ⚡ Bolt Optimization: Running `gh pr view` concurrently avoids N+1 delays.
    # Impact: Expected to reduce startup fan-out time significantly by checking up to 10 PRs concurrently.
