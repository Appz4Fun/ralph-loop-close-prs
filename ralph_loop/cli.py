...
def _check_pr_open(pr: int) -> Tuple[int, bool, Optional[Exception]]:
    try:
...
def _filter_to_still_open_prs(pr_numbers: List[int]) -> List[int]:
    ...
    kept: List[int] = []
    # ⚡ Bolt Optimization: Use concurrent execution for N+1 sequential gh calls
    with concurrent.futures.ThreadPoolExecutor(
        max_workers=min(10, len(pr_numbers))
    ) as executor:
        for pr, still_open, exc in executor.map(_check_pr_open, pr_numbers):
            if exc is not None:
                _print_step(
