...
def _check_pr_state(pr: int) -> Tuple[int, bool, Optional[Exception]]:
    try:
        ...
        return pr, True, exc  # Keep PR on transient failure
...
def _filter_to_still_open_prs(pr_numbers: List[int]) -> List[int]:
    ...
    # Use ThreadPoolExecutor to avoid N+1 sequential execution delays
    with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
        results = list(executor.map(_check_pr_state, pr_numbers))
