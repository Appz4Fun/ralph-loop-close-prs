...
def _filter_to_still_open_prs_helper(
    pr: int,
) -> Tuple[int, bool, Optional[CommandError]]:
    """Helper for concurrent execution of _pr_is_still_open."""
    try:
...
def _filter_to_still_open_prs(pr_numbers: List[int]) -> List[int]:
    ...
    # ⚡ Bolt: Execute gh pr view concurrently to avoid N+1 sequential overhead
    with concurrent.futures.ThreadPoolExecutor(
        max_workers=min(10, len(pr_numbers))
    ) as executor:
        results = list(executor.map(_filter_to_still_open_prs_helper, pr_numbers))
