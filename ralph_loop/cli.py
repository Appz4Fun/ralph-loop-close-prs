...
def _filter_to_still_open_prs(pr_numbers: List[int]) -> List[int]:
    ...
    # BOLT OPTIMIZATION: Return early if empty to prevent ValueError: max_workers must be greater than 0
    if not pr_numbers:
        return kept
    ...
    # BOLT OPTIMIZATION: Concurrently fetch PR states using ThreadPoolExecutor.
    # Expected impact: Eliminates N+1 sequential execution delays. Time taken drops from O(N) to O(N/10).
