...
def _filter_to_still_open_prs(pr_numbers: List[int]) -> List[int]:
    ...
    # Helper wrapper to catch exceptions in the worker thread and yield them back.
    ...
    # Sequential subprocess calls to `gh pr view` for many PRs creates an N+1 delay.
    # We use ThreadPoolExecutor to check PR states concurrently, reducing fan-out startup time.
    # executor.map preserves the original PR order.
    ...
    for pr, still_open, exc in results:
        if exc is not None:
            if isinstance(exc, CommandError):
                ...
                # Re-raise unexpected non-CommandError exceptions (rare)
                raise exc
            continue
