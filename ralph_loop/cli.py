...
def _check_pr_still_open_wrapper(
    pr: int,
) -> Tuple[int, bool, bool, Optional[CommandError]]:
    """Helper wrapper to evaluate PR open state, catching CommandErrors."""
    try:
        return pr, True, _pr_is_still_open(pr), None
    except CommandError as exc:
        return pr, False, False, exc
...
def _filter_to_still_open_prs(pr_numbers: List[int]) -> List[int]:
    ...
    # Use ThreadPoolExecutor to check PR states concurrently.
    # Subprocess calls to the GitHub CLI are a significant bottleneck,
    # causing an N+1 delay when evaluating PR numbers sequentially.
    with concurrent.futures.ThreadPoolExecutor(
        max_workers=min(10, len(pr_numbers))
    ) as executor:
        for pr, success, still_open, exc in executor.map(
            _check_pr_still_open_wrapper, pr_numbers
        ):
            if not success:
                _print_step(
