...
def _check_pr_state_worker(pr: int) -> Tuple[int, bool, Optional[Exception]]:
    try:
...
def _filter_to_still_open_prs(pr_numbers: List[int]) -> List[int]:
    ...
    # Optimization: Check PR states concurrently to avoid N+1 delays.
    # Expected impact: Significantly faster PR state checks when filtering multiple PRs.
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        for pr, still_open, exc in executor.map(_check_pr_state_worker, pr_numbers):
            if exc:
                if isinstance(exc, CommandError):
                    ...
                    _print_step("Unexpected error checking PR #{}: {}".format(pr, exc))
                    kept.append(pr)
            elif still_open:
                kept.append(pr)
            else:
...
def _fan_out_all_prs(
    ...
    try:
        ...
        while not shutting_down["flag"] and (children or last_exit_at):
            ...
            results = []
            if prs_to_check:
                workers = min(10, len(prs_to_check))
                # Optimization: Check PR respawn states concurrently to avoid N+1 delays.
                # Expected impact: Supervisor loop completes faster when multiple PRs are ready for respawn.
                with concurrent.futures.ThreadPoolExecutor(
                    max_workers=workers
                ) as executor:
                    results = list(executor.map(_check_pr_state_worker, prs_to_check))
