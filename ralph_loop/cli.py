...
def _filter_to_still_open_prs(pr_numbers: List[int]) -> List[int]:
    ...
    """
    # ⚡ Bolt: Optimize PR filtering using ThreadPoolExecutor
    # 📊 Impact: N sequential `gh pr view` subprocess calls are replaced with up to 10 concurrent requests.
    # This significantly reduces the fan-out spawn delay when multiple PRs are open.
    if not pr_numbers:
        return []
