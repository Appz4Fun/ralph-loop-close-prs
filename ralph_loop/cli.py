...
def _filter_to_still_open_prs(pr_numbers: List[int]) -> List[int]:
    ...
    # ⚡ Bolt: Execute gh CLI subprocess calls concurrently to avoid N+1 delays.
    # Impact: Reduces wait time from O(N) gh pr view commands to O(N / workers).
    def _check_pr(pr: int) -> Tuple[int, Any, Optional[Exception]]:
        try:
