...
def _check_single_pr(pr: int) -> Tuple[int, bool, Optional[Exception]]:
    try:
...
def _filter_to_still_open_prs(pr_numbers: List[int]) -> List[int]:
    ...
    kept: List[int] = []
    # ⚡ Bolt optimization: Fetch 'gh pr view' sequentially was causing an N+1 performance bottleneck.
    # Using ThreadPoolExecutor reduces total latency.
    # expected performance impact: Reduces fan-out verification delay significantly, scaling logarithmically rather than linearly.
    with concurrent.futures.ThreadPoolExecutor(
        max_workers=min(10, len(pr_numbers))
    ) as executor:
        results = executor.map(_check_single_pr, pr_numbers)
