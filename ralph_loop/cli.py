...
def _filter_to_still_open_prs(pr_numbers: List[int]) -> List[int]:
    ...
    def check_pr(pr_num: int) -> Tuple[int, Optional[bool], Optional[Exception]]:
        try:
            return pr_num, _pr_is_still_open(pr_num), None
        except CommandError as e:
            return pr_num, None, e

    # Parallelize PR status checks to avoid N+1 GitHub CLI calls bottleneck.
    # We yield results back sequentially and log them in the main thread to prevent interleaving.
    # Expected performance impact: Eliminates linear scaling delay for active PR counts.
