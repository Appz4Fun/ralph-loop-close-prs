...
def _filter_to_still_open_prs(pr_numbers: List[int]) -> List[int]:
    ...
    def _check_pr(pr_num: int) -> Tuple[int, bool, Optional[Exception]]:
        try:
            ...
            return (pr_num, False, exc)

    # ⚡ Bolt Optimization: Use ThreadPoolExecutor to check PR states concurrently.
    # Expected Impact: Reduces N+1 GitHub CLI delay overhead. Checking N PRs takes
    # time proportional to ~1 CLI call instead of N sequential CLI calls.
