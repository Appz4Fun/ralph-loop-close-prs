...
def _filter_to_still_open_prs(pr_numbers: List[int]) -> List[int]:
    ...
    def _check_pr_state_wrapper(
        pr: int,
    ) -> Tuple[int, Optional[bool], Optional[Exception]]:
        try:
    ...
    kept: List[int] = []
    # Using concurrent.futures to avoid N+1 sequential execution delays when fetching PR state
    max_workers = min(10, len(pr_numbers))
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        for pr, still_open, exc in executor.map(_check_pr_state_wrapper, pr_numbers):
            if exc is not None:
                _print_step(
