...
def _check_pr_still_open(pr: int) -> Tuple[int, bool, Optional[Exception]]:
    try:
...
def _filter_to_still_open_prs(pr_numbers: List[int]) -> List[int]:
    ...
    # Use ThreadPoolExecutor to run _pr_is_still_open concurrently instead of sequentially.
    # This removes an N+1 GitHub API call delay, significantly speeding up PR status checks
    # during multi-repo fan-out runs when multiple PRs are open.
    with concurrent.futures.ThreadPoolExecutor(
        max_workers=min(10, len(pr_numbers))
    ) as executor:
        for pr, still_open, exc in executor.map(_check_pr_still_open, pr_numbers):
            if exc:
                _print_step(
