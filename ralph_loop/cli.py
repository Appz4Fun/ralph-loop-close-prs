...
def _validate_pr_metadata(
    ...
    if expected_base.startswith("-"):
        raise CommandError(
            "Base branch '{}' starts with a hyphen, which is unsafe.".format(
                expected_base
            )
        )
    ...
    if pr_base:
        if pr_base.startswith("-"):
            raise CommandError(
                "PR base branch '{}' starts with a hyphen, which is unsafe.".format(
                    pr_base
                )
            )
    ...
    if branch.startswith("-"):
        raise CommandError(
            "PR head branch '{}' starts with a hyphen, which is unsafe.".format(branch)
        )
