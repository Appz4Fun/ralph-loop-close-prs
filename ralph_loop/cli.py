...
def _filter_to_still_open_prs(pr_numbers: List[int]) -> List[int]:
    ...
    """
    # ⚡ Bolt Optimization: Use ThreadPoolExecutor to check PR statuses concurrently.
    # This avoids the N+1 performance bottleneck where `gh` CLI subprocesses
    # were invoked sequentially for each PR. Expected impact: Much faster startup
    # time when checking many PRs.
    kept: List[int] = []

    def _check_pr_wrapper(pr: int) -> Tuple[int, bool, Optional[CommandError]]:
        try:
    ...
    with concurrent.futures.ThreadPoolExecutor(
        max_workers=min(10, max(1, len(pr_numbers)))
    ) as executor:
        for pr, still_open, exc in executor.map(_check_pr_wrapper, pr_numbers):
            if exc:
                _print_step(
