...
def _filter_to_still_open_prs(pr_numbers: List[int]) -> List[int]:
    ...
    def _check(pr: int) -> Tuple[int, bool, Optional[Exception]]:
        try:
    ...
    kept: List[int] = []
    # ⚡ Bolt: Execute gh operations concurrently to prevent N+1 delays.
    # Impact: Reduces wait time from O(N) subprocess spawns to O(N/max_workers).
    with ThreadPoolExecutor(max_workers=min(10, len(pr_numbers))) as executor:
        for pr, still_open, exc in executor.map(_check, pr_numbers):
            if exc:
