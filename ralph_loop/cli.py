...
def _filter_to_still_open_prs(pr_numbers: List[int]) -> List[int]:
    ...
    # BOLT: Add early return to prevent ValueError with max_workers=0
    if not pr_numbers:
        return kept
    ...
    # BOLT: Parallelized GitHub CLI calls to avoid N+1 sequential execution delays.
    with concurrent.futures.ThreadPoolExecutor(
        max_workers=min(10, len(pr_numbers))
    ) as executor:
        results = executor.map(_check_pr, pr_numbers)

    for pr, keep, exc in results:
        if exc:
            _print_step(
