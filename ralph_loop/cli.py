...
def _check_pr_state(pr: int) -> Tuple[int, bool, Optional[CommandError]]:
    try:
...
def _filter_to_still_open_prs(pr_numbers: List[int]) -> List[int]:
    ...
    """
    kept: List[int] = []
    # PERFORMANCE OPTIMIZATION: Subprocess calls to the GitHub CLI are a significant bottleneck.
    # We use ThreadPoolExecutor and map to fetch PR states concurrently while preserving original order.
    # This mitigates N+1 sequential delays during the fan-out spawn phase.
