...
def _filter_to_still_open_prs(pr_numbers: List[int]) -> List[int]:
    ...
    # N+1 calls to the GitHub CLI are a bottleneck. Running them concurrently reduces delays.
    def _check_pr_status(pr: int) -> Tuple[int, bool, Optional[Exception]]:
        try:
    ...
    kept: List[int] = []
    # Using a max of 10 workers to avoid overwhelming local resources/gh CLI while still getting concurrency benefits
    with concurrent.futures.ThreadPoolExecutor(
        max_workers=min(10, len(pr_numbers))
    ) as executor:
        for pr, still_open, exc in executor.map(_check_pr_status, pr_numbers):
            if exc:
                _print_step(
