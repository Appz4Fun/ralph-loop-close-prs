...
def _filter_to_still_open_prs(pr_numbers: List[int]) -> List[int]:
    ...
    # Subprocess calls to `gh pr view` block sequentially. Using ThreadPoolExecutor
    # allows us to verify the status of multiple PRs concurrently, significantly
    # reducing the total N+1 delay.
    def _check_pr_worker(pr: int) -> Tuple[int, bool, Optional[Exception]]:
        try:
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        results = list(executor.map(_check_pr_worker, pr_numbers))

    kept: List[int] = []
    # Process results in the main thread to preserve sequential logging order
