...
def _check_pr_state_worker(pr: int) -> Tuple[int, Optional[bool], Optional[Exception]]:
    try:
...
def _filter_to_still_open_prs(pr_numbers: List[int]) -> List[int]:
    ...
    kept: List[int] = []
    # Optimization: Use concurrent execution to avoid N+1 sequential delays
    # when verifying states of multiple PRs via the GitHub CLI.
    # Expected impact: Reduces PR filtering time from O(N) to O(N/workers).
