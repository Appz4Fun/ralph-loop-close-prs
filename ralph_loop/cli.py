...
def _filter_to_still_open_prs(pr_numbers: List[int]) -> List[int]:
    ...
    def _fetch_state(pr: int) -> Tuple[int, bool, Optional[Exception]]:
        try:
            return pr, _pr_is_still_open(pr), None
        except Exception as exc:
            return pr, False, exc

    # ⚡ Bolt Optimization: Parallelize `gh pr view` network calls.
    # Previously, checking PR statuses sequentially caused O(N) waiting delays.
    # ThreadPoolExecutor fetches states concurrently, dramatically reducing idle time.
    with concurrent.futures.ThreadPoolExecutor(
        max_workers=min(10, len(pr_numbers))
    ) as executor:
        for pr, still_open, exc in executor.map(_fetch_state, pr_numbers):
            if exc is not None:
    ...
    try:
        ...
        while not shutting_down["flag"] and (children or last_exit_at):
            ...
            if prs_to_check:
                ...
                # ⚡ Bolt Optimization: Parallelize `gh pr view` network calls for respawns.
                # Batch checks the open status of eligible PRs concurrently.
                # Avoids supervisor stalls where 1 slow request delays all subsequent PR respawns.
                with concurrent.futures.ThreadPoolExecutor(
                    max_workers=min(10, len(prs_to_check))
                ) as executor:
                    for pr, still_open, exc in executor.map(
                        _fetch_respawn_state, prs_to_check
                    ):
                        if exc is not None:
                            if isinstance(exc, CommandError):
                                _print_step(
                                ...
                                still_open_results[pr] = True
                            else:
                                raise exc
                        else:
                            still_open_results[pr] = still_open

            for pr in prs_to_check:
                still_open = still_open_results.get(pr, True)
                if not still_open:
                    _print_step(
