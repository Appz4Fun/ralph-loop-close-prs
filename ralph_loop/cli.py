...
def _filter_to_still_open_prs(pr_numbers: List[int]) -> List[int]:
    ...
    # Sequential subprocess calls to the GitHub CLI ('gh') via _pr_is_still_open
    # are a significant performance bottleneck (N+1 execution delays).
    # Concurrent execution avoids this bottleneck, preserving order with executor.map.
    # Expected Impact: PR filtering time reduced by up to 10x for large PR batches.
