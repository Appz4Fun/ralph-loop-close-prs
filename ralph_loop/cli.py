...
def _filter_to_still_open_prs(pr_numbers: List[int]) -> List[int]:
    ...
    # ⚡ BOLT OPTIMIZATION: Use ThreadPoolExecutor to prevent N+1 delays
    # Expected performance impact: Substantially reduces the wait time when checking multiple PRs
    # by making GitHub CLI calls concurrently instead of sequentially.
    def _check_pr(pr_num: int) -> Tuple[int, bool, Optional[Exception]]:
        try:
            return pr_num, _pr_is_still_open(pr_num), None
        except CommandError as e:
            return pr_num, True, e
