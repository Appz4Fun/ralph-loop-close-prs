...
def _check_pr_open_state(pr: int) -> Tuple[int, bool, Optional[Exception]]:
    try:
...
def _filter_to_still_open_prs(pr_numbers: List[int]) -> List[int]:
    ...
    kept: List[int] = []
    # ⚡ Bolt Optimization: Use ThreadPoolExecutor to prevent N+1 query latency
    # `gh pr view` subprocess calls are I/O bound. Concurrently executing them reduces fan-out latency.
    # We use `executor.map` to preserve the original order of PR numbers.
    max_workers = min(10, len(pr_numbers))
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        for pr, still_open, exc in executor.map(_check_pr_open_state, pr_numbers):
            if exc:
                _print_step(
