...
def _filter_to_still_open_prs(pr_numbers: List[int]) -> List[int]:
    ...
    # The previous sequential approach resulted in an N+1 execution pattern that degraded
    # performance linearly with the number of PRs. This concurrent model binds execution
    # time to max(network_requests) rather than sum(network_requests), dramatically
    # improving supervisor fan-out speed.
    ...
    def _check_open(pr: int) -> Tuple[int, bool, Optional[Exception]]:
        try:
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        for pr, still_open, exc in executor.map(_check_open, pr_numbers):
            if exc is not None:
                _print_step(
