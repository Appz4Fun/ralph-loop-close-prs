...
def _check_pr_open_wrapper(pr: int) -> Tuple[int, Any]:
    """Wrapper to catch exceptions and return them alongside the PR number."""
    try:
...
def _filter_to_still_open_prs(pr_numbers: List[int]) -> List[int]:
    ...
    Uses a thread pool to avoid N+1 sequential latency bottlenecks when
    spawning fan-out supervisors for large numbers of open PRs.
    """
    kept: List[int] = []
    with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
        for pr, result in executor.map(_check_pr_open_wrapper, pr_numbers):
