...
def _filter_to_still_open_prs(pr_numbers: List[int]) -> List[int]:
    ...
    # ⚡ Bolt optimization: Sequential `gh` subprocess calls for each PR are a significant bottleneck.
    # By using a ThreadPoolExecutor, we can check the state of multiple PRs concurrently,
    # drastically reducing the total wait time when dealing with many PRs.
