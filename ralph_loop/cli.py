...
def _check_pr_open_state(pr: int) -> Tuple[int, Optional[bool], Optional[Exception]]:
    """Helper to check PR open state, capturing exceptions for ThreadPoolExecutor."""
    try:
...
def _filter_to_still_open_prs(pr_numbers: List[int]) -> List[int]:
    ...
    # ⚡ Bolt: Execute gh pr view calls concurrently to avoid N+1 subprocess delays.
    # Impact: Reduces wait time from O(N) to O(N/max_workers) for open state checks.
    max_workers = min(10, len(pr_numbers))
    results = []
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        results = list(executor.map(_check_pr_open_state, pr_numbers))
