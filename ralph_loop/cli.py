...
def _filter_to_still_open_prs(pr_numbers: List[int]) -> List[int]:
    ...
    def check_pr(pr: int) -> Tuple[int, Optional[bool], Optional[Exception]]:
        try:
            return pr, _pr_is_still_open(pr), None
        except Exception as exc:
            return pr, None, exc

    # Performance Optimization: Use ThreadPoolExecutor to check PR states concurrently.
    # Why: Sequential `gh pr view` calls blocked the supervisor and wasted time on IO-bound operations (N+1 queries).
    # Impact: Reduces the fan-out preparation time, especially when managing large batches of PRs.
