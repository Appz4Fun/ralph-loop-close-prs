...
def _filter_to_still_open_prs(pr_numbers: List[int]) -> List[int]:
    ...
    # Checking each PR sequentially with a gh pr view subprocess causes N+1 delays.
    # We use executor.map to run the checks in parallel while maintaining order,
    # and sequential logging in the main thread to prevent output interleaving.
    def _check_state(pr: int) -> Tuple[int, bool, Optional[CommandError]]:
        try:
    ...
    with concurrent.futures.ThreadPoolExecutor(
        ...
        results = list(executor.map(_check_state, pr_numbers))
