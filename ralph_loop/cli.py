...
def _check_pr_open_concurrent(
    pr: int,
) -> Tuple[int, Optional[bool], Optional[CommandError]]:
    try:
...
def _filter_to_still_open_prs(pr_numbers: List[int]) -> List[int]:
    ...
    kept: List[int] = []
    # Concurrency optimization: check PR states in parallel.
    with concurrent.futures.ThreadPoolExecutor(
        max_workers=min(10, len(pr_numbers))
    ) as executor:
        for pr, still_open, exc in executor.map(_check_pr_open_concurrent, pr_numbers):
            if exc is not None:
                _print_step(
