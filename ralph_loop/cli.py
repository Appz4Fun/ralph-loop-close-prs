...
def _filter_to_still_open_prs(pr_numbers: List[int]) -> List[int]:
    ...
    def _check_pr(pr: int):
        try:
            return pr, _pr_is_still_open(pr), None
        except CommandError as exc:
            return pr, None, exc

    # Optimization: Use ThreadPoolExecutor to evaluate PRs concurrently.
    # This avoids N+1 delays by parallelizing subprocess GH CLI calls
    # while preserving the original input order using map.
