...
def _filter_to_still_open_prs(pr_numbers: List[int]) -> List[int]:
    ...
    # ⚡ Bolt Optimization:
    # _pr_is_still_open uses a blocking gh cli subprocess call. Executing these
    # sequentially (O(N)) creates a massive bottleneck for repositories with many PRs.
    # Using a ThreadPoolExecutor bounded to 10 workers speeds up this check phase
    # significantly (roughly O(N/10) elapsed time) without risking local thread exhaustion.
