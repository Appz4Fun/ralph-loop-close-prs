...
def _filter_to_still_open_prs(pr_numbers: List[int]) -> List[int]:
    ...
    """
    # ⚡ Bolt Optimization: Use ThreadPoolExecutor to prevent N+1 sequential gh subprocess execution delays.
    # Impact: Reduces wait time significantly when many PRs are being checked by doing them concurrently.
    if not pr_numbers:
        return []
