...
def _filter_to_still_open_prs(pr_numbers: List[int]) -> List[int]:
    ...
    # Optimization: Check PR states concurrently to avoid N+1 network request delays.
    # We use ThreadPoolExecutor to check up to 10 PRs in parallel, which significantly speeds up
    # fan-out initialization when there are many open PRs.
    # executor.map guarantees results are processed in the same order as pr_numbers.
