...
def _check_pr_open_worker(pr: int) -> Tuple[int, bool, Optional[Exception]]:
    try:
...
def _filter_to_still_open_prs(pr_numbers: List[int]) -> List[int]:
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        for pr, still_open, exc in executor.map(_check_pr_open_worker, pr_numbers):
            if exc is not None:
                _print_step(
            ...
            if eligible_for_respawn:
                ...
                with concurrent.futures.ThreadPoolExecutor(
                    ...
                    for pr, still_open, exc in executor.map(
                        _check_pr_open_worker, eligible_for_respawn
                    ):
