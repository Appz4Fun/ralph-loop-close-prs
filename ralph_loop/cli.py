...
def _check_pr_open_concurrently(pr: int) -> tuple[int, bool, Exception | None]:
    try:
...
def _filter_to_still_open_prs(pr_numbers: List[int]) -> List[int]:
    ...
    Optimization: Checking state for multiple PRs is a significant performance bottleneck.
    Using ThreadPoolExecutor avoids N+1 sequential execution delays.
    """
    kept: List[int] = []

    with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
        for pr, still_open, err in executor.map(_check_pr_open_concurrently, pr_numbers):
            if err:
                _print_step(
                    "Could not confirm PR #{} open state ({}); keeping it in the "
                    "fan-out set.".format(pr, err)
                )
                kept.append(pr)
                continue
            if still_open:
