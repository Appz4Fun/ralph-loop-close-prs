...
def _filter_to_still_open_prs(pr_numbers: List[int]) -> List[int]:
    ...
    # Subprocess calls to the GitHub CLI ('gh pr view') can be slow.
    # Checking them concurrently avoids an N+1 bottleneck, significantly reducing
    # the time taken during the initial fan-out phase from O(N) to O(N/max_workers).
