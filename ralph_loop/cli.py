...
def _check_pr_state_worker(pr: int) -> Tuple[int, bool, Optional[Exception]]:
    try:
        ...
        return pr, True, exc  # keep on transient error
...
def _filter_to_still_open_prs(pr_numbers: List[int]) -> List[int]:
    ...
    # ⚡ Bolt Optimization: Prevent N+1 query problem.
    # We use ThreadPoolExecutor because sequential `gh pr view` calls
    # create a major bottleneck when scanning large numbers of PRs.
    # The `executor.map` ensures PR order is preserved in the output.
    kept: List[int] = []
    max_workers = min(10, len(pr_numbers))

    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        results = executor.map(_check_pr_state_worker, pr_numbers)
